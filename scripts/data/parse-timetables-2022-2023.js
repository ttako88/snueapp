// 2022~2023 강의시간표 파서 (고학번·복학생 조회용)
//
...
const FACILITY = /^(체육관|운동장|강당|대강당|에듀웰|무용실|수영장)/;
...
function parseCell(text) {
  ...
  const cleaned = [...new Set(profs.filter((t) => !room || !t.includes(room)))];
...
for (const n of ["통섭의방법으로동화읽기", "의사소통을위한비주얼씽킹", "예비교사를위한뮤지엄여행",
  ...
  "디지털사회에서의AI작곡그리고윤리", "디지털과교사교육"]) MANUAL_TYPE.set(n, "교양");
...
fs.writeFileSync(__dirname + "/new_2022_2023.json", JSON.stringify(dedup));
console.log("\n저장: new_2022_2023.json");
