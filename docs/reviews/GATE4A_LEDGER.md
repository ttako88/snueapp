...
## 10. Gate 4a RC 최종 상태 (2026-07-21, GPT 최종 공동검수 — CODE_APPROVED)

GPT 배포 전 blocker P0-1~P0-9 + 재검수 R1~R4 전부 실측 처리. **GPT 최종 판정: 안전한 Preview UI smoke만으로 코드 RC 동결 진행 승인.**

### G. 최종 상태 라벨 (GPT 확정)
- `DB_TESTS=66/66_PASS` (dev clean replay: M/R/F/A/V/D/P/G/X/W 61 + CC 5; 80_009 behavior 무예외)
- `MAINTENANCE_MOCK=76/76_PASS` (node --test)
- `P0-7A_SERVICE_INTEGRATION=13/13_PASS` (handleMaintenance + 실제 dev 서비스클라이언트·RPC·lease·batch_runs, maintenance-e2e-dev.mjs)
- `P0-7B_HTTP_BOUNDARY=11/11_PASS` (실제 next start 서버로 route.js HTTP 시험, http-boundary-dev.mjs: disabled GET 200+no-store·POST 405·401·400·200·already_running·schema allowlist·응답/서버로그 금지값 0·lease 잔존 0)
- `LOCKFILE_REPRODUCIBILITY=PASS_WITH_IGNORE_SCRIPTS` (npm ci --ignore-scripts + npm test 76/76 + build. 순정 npm ci는 샌드박스 install-script 승인게이트로 non-fatal — Vercel Preview 순정 설치+build가 대체 증거)
- `P0-8A_UI_PREVIEW_SMOKE=PASS` (integration Preview: /api/maintenance disabled·게시판 중립문구·캘린더 토글·마법사·콘솔 0·Production/main 무변경)

### ▶️ 연기된 blocker 2건 (반드시 후속 게이트 전 해소)
- `P0-8B_PREVIEW_DEV_BINDING=DEFERRED_GATE4B5_BLOCKER` — Preview가 운영 NEXT_PUBLIC 상속 중. Gate 4b/5(로그인·게시판 CRUD·Auth) 착수 전 반드시 Preview를 dev로 재지정. 그때까지 Preview 안전조건: ①Deployment Protection 유지 ②MAINTENANCE_ENABLED=false 유지 ③/board/*·/login·회원가입·글쓰기·추천 등 DB 연결 화면 시험 금지 ④Preview URL 외부배포 금지 ⑤Preview를 "dev 백엔드"로 표시 금지.
- `DELETE_ACCOUNT_SYNTHETIC_E2E=DEFERRED_CRON_ENABLE_BLOCKER` — delete-accounts의 §13 안전게이트를 합성 fixture(실패게이트/정상삭제)로 실증하는 시험은 미완. **운영 Cron 활성화(Phase 8) 전 필수.** (P0-7 실측의 delete-accounts failed=1은 "실제 dev Route 코어 호출에서 실패 상태로 안전 종료·응답 비식별"까지만 실증이며, Auth 안전게이트 E2E 실증은 아님 — mock 검증과 구분.)

### 산출물·SHA
- RC 후보 코드: `f5fc4ec` = **CODE_APPROVED**. reset 원문 SHA-256 `4b0ab5d8747d907de143b49abcf50d5f82671e5697eba28675981de147221739`, 009 원문 `faff5f85…`.
- dev 실행 도구(로컬, .env.dev.local의 DEV_DB_URL·SUPABASE_SECRET_KEY만 참조·비출력): apply-sql-dev.mjs·maintenance-e2e-dev.mjs·http-boundary-dev.mjs·boot-concurrency-dev.mjs·dev-url.mjs(공용 검증기).
- dev 실측 발견·수정 결함(누적): reset char캐스트·authz._log allowlist·fixture uuid::text·(그 외 001~008 동결 전 3건). 전부 fail-closed로 파괴 전 검출.
- **FINAL_RC_SHA = 본 문서 정정 커밋 SHA** (runtime 코드·SQL·vercel.json 무변경, secret 0, origin/main=bc7fa65 유지, Production deployment/env 무변경).

### 다음 = 운영 배포 (사용자 명시 승인 + GPT 런북 Phase 1~8, 각 파괴 단계 지정 문구)
BACKUP-OK / GO-RESET-PROD / GO-DEPLOY-RC / OWNER-BOOTSTRAP-OK / GO-ENABLE-PROD-CRON. Cron 활성화(Phase 8) 전 DELETE_ACCOUNT_SYNTHETIC_E2E 해소 필수.
