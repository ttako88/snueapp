...
//
// GPT 공동검수가 dev 승격 차단 조건으로 지정한 **7 시나리오**:
//   1. 010 잠금해제 동시 요청 — 과금 1회, 잔액 0
//   2. 010 동일 평가 동시 정정 — 대기본 1개만
//   3. 011 게시판 고정 상한 동시 요청 — 활성 공지 최대 5개
//   4. 012 일반 신고 동시 요청 — 서로 다른 3명 기준 정확히 1회 숨김
//   5. 012 긴급 자동숨김 동시 요청 — 신고자별 24시간 3건 상한
//   6. 012 투표 동시 전환 — 행·카운터 정합
//   7. 013 버그제보 4건 동시 제출 — 최대 3건 접수
//
// ⚠️ 미적용 상태의 exit 3은 **BLOCKED_NOT_APPLIED이지 PASS가 아니다.**
//
...
async function preflight(c) {
  ...
  if (!(r.m010 && r.m010b && r.m011 && r.m012)) {
    console.error("[BLOCKED_NOT_APPLIED] 010~013이 dev에 적용돼 있지 않습니다. 통과가 아니라 \"실행 불가\"입니다.");
