...
| P2 | **사용자의 dev 리허설 착수 승인** — dev 프로젝트 한정, 운영 무접촉 | 대기 (기상 후 질문 2) |
| P3 | SNUE 학번 형식 확인 — **계획·골격 작성은 비차단.** 검증 규칙은 설정값+명시적 TODO로 두되, **신원 관련 함수 확정·신원 테스트·운영 적용 전에는 반드시 해소** | 대기 (기상 후 질문 1) |
...
### 단계 A — dev 리허설 (운영 무접촉)
1. dev 프로젝트 ID 재확인 → 작업 로그에 "DEV" 명시
2. provision-storage 실행 (dev) → public=false·MIME·크기 제한 검증
3. 001→002→003→004 순차 적용
...
13. **dev에서 검증된 동일 커밋 SHA의 파일만 적용** — 운영에서 즉석 수정 금지, 미검증 로컬 파일 실행 금지. 001~004 → provision-storage(운영)
