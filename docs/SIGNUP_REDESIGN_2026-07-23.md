...
> **상태: PROD 라이브 + 실동작 확인 (2026-07-23).** migration 030 운영적용 완료(APPLY 재실행이라 객체증가·anon검사 헛FAIL, 실제 적용됨: account_identity +5함수, 데이터불변). 코드 배포(main=6673d32). check-hakbeon 체인 스모크 PASS(HMAC 키 존재·svc_hakbeon_exists 작동). 남음: 029 적용(analytics), 콘솔 username 표시(migration 031, 후속).
